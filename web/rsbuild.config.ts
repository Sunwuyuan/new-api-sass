...
export default defineConfig(({ envMode }) => {
  ...
  // Workspace and platform pages stay on the Rsbuild SPA; API/relay paths
  // are forwarded to the Go gateway with the original Host so wildcard and
  // custom domains resolve to the matching workspace.
  ...
  const devProxy = {
    ...Object.fromEntries(
      (['/api', '/v1', '/mj', '/pg', '/suno'] as const).map((key) => [
        key,
        gatewayProxy,
      ])
    ),
