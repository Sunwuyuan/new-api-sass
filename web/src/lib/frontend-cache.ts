...
export function initializeFrontendCache(): void {
  ...
  try {
    const currentVersion = tenantStorage.getItem(FRONTEND_CACHE_VERSION_KEY)
    if (currentVersion === FRONTEND_CACHE_VERSION) return

    clearLocalUiCache()
    tenantStorage.setItem(FRONTEND_CACHE_VERSION_KEY, FRONTEND_CACHE_VERSION)
  } catch {
...
function clearLocalUiCache(): void {
  ...
  for (let index = 0; index < tenantStorage.length; index += 1) {
    const key = tenantStorage.key(index)
    if (key && !PRESERVED_LOCAL_STORAGE_KEYS.has(key)) {
      keysToRemove.push(key)
    }
  }

  keysToRemove.forEach((key) => tenantStorage.removeItem(key))
}
