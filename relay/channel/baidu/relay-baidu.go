...
func getBaiduAccessToken(tenantCtx context.Context, apiKey string) (string, error) {
	if val, ok := baiduTokenStore.Load(apiKey); ok {
		...
		if accessToken, ok = val.(BaiduAccessToken); ok {
			...
			if time.Now().Add(time.Hour).After(accessToken.ExpiresAt) {
				go func() {
					_, _ = getBaiduAccessTokenHelper(tenantCtx, apiKey)
				}()
			}
			return accessToken.AccessToken, nil
		}
	}
	accessToken, err := getBaiduAccessTokenHelper(tenantCtx, apiKey)
	if err != nil {
		return "", err
	}
	if accessToken == nil {
...
func getBaiduAccessTokenHelper(tenantCtx context.Context, apiKey string) (*BaiduAccessToken, error) {
	...
	res, err := service.GetHttpClient(tenantCtx).Do(req)
	if err != nil {
		return nil, err
	}
