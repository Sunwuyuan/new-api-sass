...
// TwoFA 用户2FA设置表
type TwoFA struct {
	TenantID       int64          `json:"-" gorm:"not null;index;uniqueIndex:tenant_two_f_a_user_id,priority:1"`
	Id             int            `json:"id" gorm:"primaryKey"`
	UserId         int            `json:"user_id" gorm:"uniqueIndex:tenant_two_f_a_user_id;not null;index"`
...
func GetTwoFAByUserId(tenantCtx context.Context, userId int) (*TwoFA, error) {
	if userId == 0 {
		return nil, errors.New("用户ID不能为空")
	}

	var twoFA TwoFA
	err := DB.WithContext(tenantCtx).Where("user_id = ?", userId).First(&twoFA).Error
	if err != nil {
...
func IsTwoFAEnabled(tenantCtx context.Context, userId int) (bool, error) {
	twoFA, err := GetTwoFAByUserId(tenantCtx, userId)
	if err != nil {
		return false, err
	}
...
func (t *TwoFA) updateUsageState(tenantCtx context.Context) error {
	if t.Id == 0 {
		return errors.New("2FA记录ID不能为空")
	}
	return DB.WithContext(tenantCtx).Model(&TwoFA{}).Where("id = ?", t.Id).Updates(map[string]any{
...
func (t *TwoFA) ResetFailedAttempts(tenantCtx context.Context) error {
	t.FailedAttempts = 0
	t.LockedUntil = nil
	return t.updateUsageState(tenantCtx)
}

// IncrementFailedAttempts 增加失败尝试次数
func (t *TwoFA) IncrementFailedAttempts(tenantCtx context.Context) error {
	if t.Id == 0 {
		return errors.New("2FA记录ID不能为空")
	}
	...
	for range maxUpdateRetries {
		var current TwoFA
		if err := DB.WithContext(tenantCtx).Select("id", "failed_attempts", "locked_until").First(&current, t.Id).Error; err != nil {
			return err
		}
		...
		result := DB.WithContext(tenantCtx).Model(&TwoFA{}).
...
func ReplaceBackupCodesWithAuthVersion(tenantCtx context.Context, userId int, codes []string) error {
	return replaceBackupCodesWithAuthVersion(tenantCtx, userId, codes, nil)
}

func ReplaceBackupCodesForSession(tenantCtx context.Context, identity AuthSessionIdentity, codes []string) error {
	return replaceBackupCodesWithAuthVersion(tenantCtx, identity.UserID, codes, &identity)
}

func replaceBackupCodesWithAuthVersion(tenantCtx context.Context, userId int, codes []string, identity *AuthSessionIdentity) error {
...
func ValidateBackupCode(tenantCtx context.Context, userId int, code string) (bool, error) {
	...
	if err := DB.WithContext(tenantCtx).Where("user_id = ? AND is_used = false", userId).Find(&backupCodes).Error; err != nil {
		return false, err
	}
	...
	for _, bc := range backupCodes {
		if common.ValidatePasswordAndHash(normalizedCode, bc.CodeHash) {
			now := time.Now()
			result := DB.WithContext(tenantCtx).Model(&TwoFABackupCode{}).
...
func GetUnusedBackupCodeCount(tenantCtx context.Context, userId int) (int, error) {
	var count int64
	err := DB.WithContext(tenantCtx).Model(&TwoFABackupCode{}).Where("user_id = ? AND is_used = false", userId).Count(&count).Error
...
func DisableTwoFAWithAuthVersion(tenantCtx context.Context, userId int) error {
	return disableTwoFAWithAuthVersion(tenantCtx, userId, nil)
}

func DisableTwoFAForSession(tenantCtx context.Context, identity AuthSessionIdentity) error {
	return disableTwoFAWithAuthVersion(tenantCtx, identity.UserID, &identity)
}

func disableTwoFAWithAuthVersion(tenantCtx context.Context, userId int, identity *AuthSessionIdentity) error {
	if err := DB.WithContext(tenantCtx).Transaction(func(tx *gorm.DB) error {
		if identity != nil {
			if err := ValidateAuthSessionWithTx(tx, *identity); err != nil {
				return err
...
func (t *TwoFA) ValidateTOTPAndUpdateUsage(tenantCtx context.Context, code string) (bool, error) {
	// 检查是否被锁定
	if t.IsLocked() {
	...
	if !common.ValidateTOTPCode(t.Secret, code) {
		// 增加失败次数
		if err := t.IncrementFailedAttempts(tenantCtx); err != nil {
			return false, err
		}
		return false, nil
	}
	...
	if err := t.updateUsageState(tenantCtx); err != nil {
		return false, err
	}
...
func (t *TwoFA) ValidateBackupCodeAndUpdateUsage(tenantCtx context.Context, code string) (bool, error) {
	// 检查是否被锁定
	if t.IsLocked() {
		return false, fmt.Errorf("账户已被锁定，请在%v后重试", t.LockedUntil.Format("2006-01-02 15:04:05"))
	}
	...
	// 验证备用码
	valid, err := ValidateBackupCode(tenantCtx, t.UserId, code)
	if err != nil {
		return false, err
	}
...
func GetTwoFAStats(tenantCtx context.Context) (map[string]any, error) {
	var totalUsers, enabledUsers int64

	// 总用户数
	if err := DB.WithContext(tenantCtx).Model(&User{}).Count(&totalUsers).Error; err != nil {
		return nil, err
	}

	// 启用2FA的用户数
	if err := DB.WithContext(tenantCtx).Model(&TwoFA{}).Where("is_enabled = true").Count(&enabledUsers).Error; err != nil {
		return nil, err
	}
