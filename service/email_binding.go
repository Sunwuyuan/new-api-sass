...
func ValidateAccountEmail(tenantCtx context.Context, email string) (string, error) {
	...
	if common.TenantState(tenantCtx).EmailDomainRestrictionEnabled {
		allowed := slices.Contains(common.TenantState(tenantCtx).EmailDomainWhitelist, parts[1])
		if !allowed {
			return "", ErrAccountEmailRestricted
		}
	}
	if common.TenantState(tenantCtx).EmailAliasRestrictionEnabled && strings.ContainsAny(parts[0], "+.") {
		return "", ErrAccountEmailRestricted
	}
	return email, nil
}

func StartEmailBinding(tenantCtx context.Context, identity AuthIdentity, authorization *model.AuthFlowAuthorization, email string) (*EmailBindingData, error) {
	email, err := ValidateAccountEmail(tenantCtx, email)
	if err != nil {
		return nil, err
	}
	...
	if err := ValidateFlowAuthorization(tenantCtx, identity, VerificationOperation{Scope: VerificationScopeAccountBind, Context: context}, authorization); err != nil {
		return nil, err
	}
	...
	token, flow, err := model.CreateEmailBinding(tenantCtx, identity, state)
	if err != nil {
		return nil, err
	}
	if err := sendEmailBindingCodes(tenantCtx, state, codes); err != nil {
		// A partially delivered pair must not leave a usable change request.
		_, _ = model.ConsumeAuthFlow(tenantCtx, token, model.AuthFlowMatch{Purpose: model.AuthFlowPurposeEmailBinding, UserId: identity.UserID, SessionId: identity.SessionID})
		return nil, ErrEmailBindingDelivery
	}
	data := emailBindingData(token, flow, &state)
	if state.CurrentEmail != "" && !requireOld {
		data.NotificationWarning = NotifyAccountSecurityChange(tenantCtx, state.CurrentEmail, "A change of your email address was requested") != nil
	}
	return data, nil
}

func ResendAccountEmailBinding(tenantCtx context.Context, identity AuthIdentity, token string) (*EmailBindingData, error) {
	_, state, err := model.GetEmailBinding(tenantCtx, identity, token)
	if err != nil {
		return nil, err
	}
	...
	if err := ValidateFlowAuthorization(tenantCtx, identity, VerificationOperation{Scope: VerificationScopeAccountBind, Context: context}, state.Authorization); err != nil {
		return nil, err
	}
	...
	flow, state, err := model.ResendEmailBinding(tenantCtx, identity, token, codes.NewHash, codes.OldHash)
	if err != nil {
		return nil, err
	}
	if err := sendEmailBindingCodes(tenantCtx, *state, codes); err != nil {
...
func FinishEmailBinding(tenantCtx context.Context, identity AuthIdentity, token, newCode, oldCode string) (*model.EmailBindingState, error) {
	_, state, err := model.GetEmailBinding(tenantCtx, identity, token)
	if err != nil {
		return nil, err
	}
	...
	if _, err := ValidateAccountEmail(tenantCtx, state.Email); err != nil {
		return nil, err
	}
	return model.CompleteEmailBinding(tenantCtx, identity, token, state.Email, newCode, oldCode)
}
...
func sendEmailBindingCodes(tenantCtx context.Context, state model.EmailBindingState, codes emailBindingCodes) error {
	subject := common.TenantState(tenantCtx).SystemName + " — Confirm your email address"
	...
	if err := common.SendEmail(tenantCtx, subject, state.Email, content); err != nil {
		return err
	}
	if codes.Old != "" {
		...
		return common.SendEmail(tenantCtx, subject, state.CurrentEmail, content)
	}
	return nil
}
